...
export function roboFromCommand(command: string, params: IVacuumCommand, device: VacuumDevice) : IRoboVacuumCommand {
	...
	switch (command) {
		...
		case "action.devices.commands.StartStop" : {
			...
			const segment = device.segments[zones[0]]
			if (segment) 
				return {
					id: id,
					method: "app_segment_clean",
					params: zones.flatMap(zone=>device.segments[zone].segments)
				}
			const dzone = device.zones[zones[0]]
			if (dzone) 
				return {
					id: id,
					method: "app_zoned_clean",
					params: zones.flatMap(zone=>device.zones[zone].zones)
				};
			const target = device.targets[zones[0]]
			if (target) 
				return {
					id: id,
